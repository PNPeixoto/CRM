...
@Component
class ReservaDeReconciliacaoTelegram {

    private final EntityManager entityManager;
    private final int tamanhoDoLote;
    private final int intervaloSegundos;

    ReservaDeReconciliacaoTelegram(
            EntityManager entityManager,
            @Value("${app.providers.telegram.reconciliation-batch-size:25}") int tamanhoDoLote,
            @Value("${app.providers.telegram.reconciliation-interval-seconds:300}") int intervaloSegundos) {
    ...
    List<CanalReservado> proximoLote() {
        List<Tuple> linhas = entityManager.createNativeQuery("""
                SELECT connection_id, connection_tenant_id
                  FROM reservar_canais_telegram_para_reconciliar(:limite, :intervalo)
                """, Tuple.class)
